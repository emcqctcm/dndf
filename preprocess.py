...
import pywt
def wavelet_FCNN_preprocessing_set(X, waveletLevel=3-1, waveletFilter='haar'):
    X=X.squeeze()
    N = X.shape[0]
...
def get_data(path, subject, LOSO = False, isStandard = True):
    ...
    if LOSO:
        ...
        X_train, y_train = load_data(path, subject+1, True)  #(288,1,22,1125)
        X_test, y_test = load_data(path, subject+1, False)  #(288,1,22,1125)
        # X_train=np.array(X_train)
        # X_test=np.array(X_test)
        # print('NO_LOSO')
        # X_train=X_train.reshape(288,22,1750)
        # X_test=X_train.reshape(288,22,1750)
        # X_train=wavelet_FCNN_preprocessing_set(X_train)
        # X_test=wavelet_FCNN_preprocessing_set(X_test)
        # X_train=np.array(X_train)
        # X_test=np.array(X_test)
        # X_train=X_train.reshape(288,22,66)
        # X_test=X_train.reshape(288,22,66)
...
dataset_conf = { 'n_classes': 4, 'n_sub': 1, 'n_channels': 22, 'data_path': data_path,  #'n_sub': 9,
            'isStandard': True, 'LOSO': False}
# Set training hyperparamters
train_conf = { 'batch_size': 64, 'epochs': 1, 'patience': 300, 'lr': 0.0009,  #'epochs': 1000
...
for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
    ...
    print(X_train.shape,y_train_onehot.shape)  #(288, 1, 22, 1125) (288, 4)
    print(type(X_train))
#    plt.figure(figsize=(18,12))
#    for i in range(22):        
#        plt.subplot(22,1,i+1)
#        
#        plt.plot(X_train[1,0,i,:])
#        # plt.axis('off')        
#        # plt.xticks([])
#        plt.yticks([])
#    # plt.subplot(21,1,i+1)
#    # plt.plot(X_train[1,0,21,:])
#    # plt.axis('off')
#    # plt.xticks(range(1125))
#    plt.xlabel('(a)')
#    
#    plt.show()
